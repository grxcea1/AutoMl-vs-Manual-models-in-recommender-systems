...
def print_results(model_name, runtime, mse, rmse, mae, precision, recall, f1, prec_at_k, rec_at_k):
  ...
  print(f" Precision (full) :  {precision :.4f}") #print precison to 4dp
  print(f" Recall (full):      {recall:.4f}") #print recall  to 4dp
...
def save_comparison_chart(results): #this stores the table as a dictionary with the model and its metrics
    ...
    metrics = [
        ("Precision", "Precision",      "Which model had top relevance?"),
        ("RMSE",      "RMSE",           "Which model predicted ratings most accurately?"),
        ("Runtime",   "Runtime (s)",    "Which model was fastest?"),
        ("Precision@k", "Precision@10", "Which model gave the most relevant top 10?"),
...
def run():  
    ...
    mse, rmse, mae, precision, recall, f1 = evaluate_model(y_test, y_pred_nb) #stores the results in the dictionary
    prec_k, rec_k = show_top10(nb_model, preprocess, 'Naive Bayes', X_test, y_test, sample_users_ids=[1, 2, 3]) #this runs the evaluation for the naive bayes model and stores precision@10 and recall@10 results
    #stores the results that we have gotten inside the diction we made in the comparison table function
   ...
   #prints the results
    ...
    mse, rmse, mae, precision, recall, f1 = evaluate_model(y_test, y_pred_mlp) #stores the results in the dictionary
    prec_k, rec_k = show_top10(mlp_model, preprocess, 'MLP', X_test, y_test, sample_users_ids=[1, 2, 3])
    #stores the results that we have gotten inside the diction we made in the comparison table function
    results['MLP'] = { 'MSE': mse,
        'RMSE': rmse, 'MAE': mae,
        'Precision': precision, 'Recall': recall,
